...
class LightningFinetunedModel(pl.LightningModule):
    ...
    def on_fit_start(self) -> None:  # type: ignore[override]
        ...
        try:
            ...
            dummy_obs = torch.randn(1, model_config['n_cells'], model_config['n_genes'], device=self.device)
            dummy_gt = torch.randn(1, model_config['n_cells'], model_config['n_genes'], device=self.device)
